...
ntpl.setFile array( _
	...
	,array("MAIN"   , "mypage/list.html" ) _
	,array("FOOTER" , "_inc/footer.html") _
), ""
'// 상단 로그인 블럭처리
Call loginBlock_ntpl("HEADER","LOGIN_BOX","LOGOUT_BOX")

call ntpl.setBlock("MAIN", array("BOARD_LOOP" , "BOARD_LOOP_NODATA","BOARD_BTN_WRITE" , "MYJUDGE_LIST"))
'왼쪽 심사위원등록 메뉴
...
Sub getList()
	...
	"DECLARE @pageNo INT , @rows INT ,@BoardKey INT ,@UserIdx INT ;" &_
	"SET @UserIdx     = ?; " &_
	"SET @pageNo     = ?; " &_
	"SET @rows      = ?; " &_
	"SET @BoardKey  = ?; " &_
