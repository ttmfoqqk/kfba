...
Else
	If PhotoExt = "jpg" Or PhotoExt = "jpeg" Or PhotoExt = "gif" Or PhotoExt = "png" Or PhotoExt = "bmp" Then
		...
		PhotoDownBtn = "<a href=""download.asp?pach=" & USER_PHOTO_PATH & "&file=" & FI_Photo & "&name=" & Server.URLEncode(FI_UserName) &"."&PhotoExt & """>[사진 다운로드]</a>"
	Else
		UserPhotos= "<a href="""&USER_PHOTO_PATH &  FI_Photo &""">"& FI_Photo &"</a>"
		PhotoDownBtn = ""
	End If
End If
...
Sub getView()
	...
	"SET @Idx = ? ;" &_
	"SET @UserIdx = (SELECT [UserIdx] FROM [dbo].[SP_PROGRAM_APP] WHERE [Idx] = @Idx ) ; " &_
	"SELECT " &_
	"	 A.[Idx] " &_
	"	,A.[UserIdx] " &_
